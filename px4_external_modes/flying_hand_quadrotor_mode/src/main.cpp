...
#include <flying_hand_quadrotor_mode/flying_hand_controller.hpp>
...
int main(int argc, char * argv[])
{
  ...
  try {
    auto node = std::make_shared<rclcpp::Node>("flying_hand_quadrotor_mode");
    ...
    auto controller = std::make_shared<flying_hand_quadrotor_mode::FlyingHandController>(*node);
    flying_hand_control_common::ControllerCallbacks controller_callbacks;
    ...
    const auto arm_kinematics =
      std::make_shared<flying_hand_quadrotor_mode::ArmKinematics>();
    ...
    auto mode = std::make_shared<flying_hand_control_common::FlyingHandMode>(
      *node, std::move(mode_configuration),
      [arm_kinematics](const flying_hand_control_common::JointVector & joints) {
    ...
    RCLCPP_FATAL(rclcpp::get_logger("flying_hand_quadrotor_mode"), "%s", exception.what());
    rclcpp::shutdown();
    return 1;
  }
