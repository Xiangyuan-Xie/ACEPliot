...
#include <trajectory_generators/rc_manual_generator.hpp>
#include "../../../rl_base_mode/include/robot_data.hpp"
#include <px4_ros2/utils/frame_conversion.hpp>
#include <algorithm>
#include <cmath>

RcManualGenerator::RcManualGenerator(px4_ros2::ModeBase & mode)
: manual_control_input_(std::make_shared<px4_ros2::ManualControlInput>(mode)) {}
...
void RcManualGenerator::reset(const RobotData & robot_data)
{
  ...
  last_pos_w_xy_ = robot_data.RootPosW().head<2>().cast<float>();
}

TrajectorySample RcManualGenerator::step(float dt, const RobotData & robot_data)
{
  ...
  const float v_xy_body = robot_data.RootLinVelB().head<2>().norm();
  const bool stopped_xy = (p_.hold_max_xy < 1e-6f) || (v_xy_body < p_.hold_max_xy);

  const Eigen::Vector2f pos_w_xy = robot_data.RootPosW().head<2>().cast<float>();
  ...
  const float vz_body = robot_data.RootLinVelB().z();
  const bool stopped_z = (p_.hold_max_z < 0.3f) || (std::fabs(vz_body) < p_.hold_max_z);

  const float z_w = robot_data.RootPosW().z();
