...
static constexpr char kArmPositionCTBRModeName[] = "RL Arm Position CTBR";
 ...
 */
class RlMCArmPositionCTBRMode : public RlMCArmPositionDirectActuatorsMode
{
public:
  /**
   ...
   */
  explicit RlMCArmPositionCTBRMode(
    rclcpp::Node & node,
    const std::string & mode_name = kArmPositionCTBRModeName,
    bool activate_disarmed = kArmPositionActivateEvenWhileDisarmed,
    const std::string & topic_namespace_prefix = "",
    const std::string & root_dir = ROOT_DIR);

  /// @brief Default destructor.
  ~RlMCArmPositionCTBRMode() override = default;
