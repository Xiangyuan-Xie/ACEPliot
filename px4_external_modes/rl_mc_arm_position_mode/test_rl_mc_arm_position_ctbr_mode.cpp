...
TEST(RlMcArmPositionCtbrMode, ZeroRawActionMapsToHoverCollective)
{
...
TEST(RlMcArmPositionCtbrMode, PositivePitchAndYawFlipIntoFrd)
{
  const CtbrSetpointFrd setpoint = mapRawCtbrActionToPx4Setpoint(
...
TEST(RlMcArmPositionCtbrMode, MetadataLoaderParsesCtbrFields)
{
  const std::filesystem::path metadata_path =
    std::filesystem::temp_directory_path() / "rl_mc_arm_position_ctbr_metadata_test.json";
