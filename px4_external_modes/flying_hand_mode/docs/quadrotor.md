...
The `flying_hand_mode` quadrotor controller is a dynamically registered PX4 ROS 2 External Mode for the
...
tilted-rotor fully actuated aircraft. Use the fully actuated launch in the same
package for the separately calibrated six-rotor architecture reconstruction.
...
colcon build --packages-select acados_vendor px4_msgs px4_ros2_cpp flying_hand_mode
colcon test --packages-select flying_hand_mode
```
...
echo "1973ddd1b536dcd4a059a22f7259c3b0e6b6c878cbfbe7e9b962eba195f361d9  $renderer" \
  | sha256sum -c -
chmod +x "$renderer"
python3 -m pip install -r px4_external_modes/flying_hand_mode/tools/requirements-quadrotor-codegen.txt
export ACADOS_SOURCE_DIR=$PWD/third_party/acados_vendor/vendor/acados
python3 px4_external_modes/flying_hand_mode/tools/generate_quadrotor_solver.py
```
...
```bash
python3 px4_external_modes/flying_hand_mode/tools/generate_quadrotor_control_urdf.py \
