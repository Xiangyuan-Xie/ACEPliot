#include <flying_hand_control_common/external_mode.hpp>

#include <flying_hand_fully_actuated_mode/flying_hand_fully_actuated_controller.hpp>
...
int main(int argc, char * argv[])
{
  ...
  try {
    auto node = std::make_shared<rclcpp::Node>("flying_hand_fully_actuated_mode");
    ...
    auto controller = std::make_shared<
      flying_hand_fully_actuated_mode::FlyingHandFullyActuatedController>(*node);
    flying_hand_control_common::ControllerCallbacks callbacks;
    callbacks.update = [controller](const flying_hand_control_common::ControllerInput & input) {
    ...
    flying_hand_control_common::FlyingHandModeConfiguration mode_configuration;
    ...
    auto mode = std::make_shared<flying_hand_control_common::FlyingHandMode>(
      *node, std::move(mode_configuration),
      [controller](const flying_hand_control_common::JointVector & joints) {
        return controller->endEffectorPoseFlu(joints);
      },
      px4_prefix, std::move(callbacks));
    auto executor =
      std::make_unique<flying_hand_control_common::FlyingHandModeExecutor>(*mode, px4_prefix);
    ...
    const std::weak_ptr<flying_hand_control_common::FlyingHandMode> weak_mode = mode;
    ...
    RCLCPP_FATAL(
      rclcpp::get_logger("flying_hand_fully_actuated_mode"), "%s", exception.what());
    rclcpp::shutdown();
    return 1;
  }
