...
void Figure8Generator::reset(const TrajectoryGeneratorState & state)
{
  // Record starting pose & heading
  start_pos_ = state.root_pos_w;
...
TrajectorySample Figure8Generator::step(float dt, const TrajectoryGeneratorState & state)
{
  t_ += dt;
  ...
  switch (state_) {
    ...
    case State::CLIMBING: {
        const float z_now = state.root_pos_w.z();
